...
`@codex` メンションによる Codex CLI 連携を使用する場合は、以下のいずれかのシークレットを追加で登録します。
...
**役割**: Codex CLI のセットアップと実行を行う共通アクションです。`run-claude`、`run-claude-plan`、`run-claude-review`、`run-claude-breakdown` が `mention_type=codex` の場合に内部的に使用します。GitHub 連携機能（進捗コメント投稿、コード変更の自動コミット・プッシュ）を備えています。
...
| `model` | Codex で使用するモデル名（例: `o4-mini`, `o3`, `gpt-4.1`）。 |
| `reasoning_effort` | o-series モデルの推論レベル（`low`, `medium`, `high`）。 |
